...
export const mapNewSearchToOld = (res: SearchItem, suffix = false) => {
  ...
  if (
    ...
    res.price_promotion_info.final_promotion_path_list.final_promotion_path_map_data.find(
      (item) =>
        item.promotion_title === '商品券' || item.promotion_title === '店铺券'
    )
  ) {
    const coupon =
      res.price_promotion_info.final_promotion_path_list
    ...
    coupon_start_time = dayjs(Number(coupon.promotion_start_time)).format(
      'YYYY-MM-DD HH:mm:ss'
    );
    coupon_end_time = dayjs(Number(coupon.promotion_end_time)).format(
      'YYYY-MM-DD HH:mm:ss'
    );
...
export const parseTbResult = (item: any, rate = 1) => {
  ...
  const res = {
    ...
    commission: Number(
      (((promotionPrice * commissionRate) / 100 / 100) * rate).toFixed(2)
    ),
    sales: _item.volume,
  };
  return res;
...
export const handlePwd = (response: any) => {
  ...
  if (isValidInfo) {
    ...
    const bestPwd =
      parseInfo.coupon_info_dto && Object.keys(parseInfo.coupon_info_dto).length
    ...
    const res: any = {
      ...
      info: {
        ...
        model:
          ...
          parseInfo.link_info_dto.cps_short_tpwd,
        password:
          parseInfo.link_info_dto.coupon_full_tpwd ||
          parseInfo.link_info_dto.cps_full_tpwd,
        short_url:
          parseInfo.link_info_dto.coupon_short_url ||
    ...
    if (parseInfo.link_info_dto.material_type === 1 && res.info.originUrl) {
      const match = res.info.originUrl.match(
        /activityId=([\w\d]+?)(?:$|\b|\s|&)/
      );
      if (match) {
